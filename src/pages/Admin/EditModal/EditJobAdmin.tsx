...
import { JobItemInterface, postEditJobApi } from '../../../redux/reducers/admin/UserJobReducer'
import { DispatchType, RootState } from '../../../redux/configStore'
import { useDispatch, useSelector } from 'react-redux'
import { useFormik } from 'formik';
import * as yup from 'yup';
import { JobAdminCreateForm } from '../CreateModal/CreateJobModal';
import { CategoryJobInterface } from '../../../redux/reducers/admin/categoryAdminReducer';
type Props = {
    prod:JobItemInterface
}

const EditJobAdmin = ({prod}: Props) => {
    ...
    const frm = useFormik<JobAdminCreateForm>({
        initialValues: {
            tenCongViec: prod.tenCongViec,
            danhGia: prod.danhGia,
            giaTien: prod.giaTien,
            maChiTietLoaiCongViec: prod.maChiTietLoaiCongViec,
            moTa: prod.moTa,
            moTaNgan: prod.moTaNgan,
            saoCongViec: prod.saoCongViec,
        ...
        onSubmit: async (values: JobAdminCreateForm) => {
            console.log(values)
            const actionasync = postEditJobApi(values, prod.id);
            dispatch(actionasync).then(() => {closeModalAndReloadPage()});
        },
    });
    const closeModalAndReloadPage = async () => {
        // Close the modal
        const modal = document.getElementById(`EditJobModal${prod.id}`);
    ...
    return (
        <div className="modal fade" id={`EditJobModal${prod.id}`} tabIndex={-1} data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-labelledby="modalTitleId" aria-hidden="true">
